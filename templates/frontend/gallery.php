  ...
  if ('1' === $this->options['show_breadcrumb'] || '1' === $this->options['search'] || '1' === $this->options['show_refreshbutton']
      ...
      if ('0' === $this->options['single_account']) {
          ...
          foreach ($this->get_main()->get_accounts()->list_accounts() as $account_id => $account) {
              echo "<div class='nav-account-selector' data-account-id='{$account->get_id()}' title='{$account->get_name()} ({$account->get_email()})'><div><img src='{$account->get_image()}'/><span>{$account->get_name()} ({$account->get_email()})</span></div></div>\n";
          }
          echo '</div>';
      } ?></div><?php
  } ?>
  <div class="file-container">
    ...
    <div class="ajax-filelist" style="<?php echo (!empty($this->options['maxheight'])) ? 'max-height:'.$this->options['maxheight'].';overflow-y: scroll;' : ''; ?>">
      <div class='images image-collage'>
        ...
        \TheLion\UseyourDrive\Skeleton::get_gallery_placeholders($target_height, 5);
        ?>
      </div></div>
