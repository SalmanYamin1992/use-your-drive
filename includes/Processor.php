...
class Processor
{
    ...
    public function start_process()
    {
        ...
        if (null === $this->get_current_account() || false === $this->get_current_account()->get_authorization()->has_access_token()) {
            ...
            return new \WP_Error('broke', '<strong>'.sprintf(esc_html__('%s needs your help!', 'wpcloudplugins'), 'Use-your-Drive').'</strong> '.esc_html__('Authorize the plugin!', 'wpcloudplugins').'.');
        }
        ...
        if (!empty($this->options['subfolder']) && '/' !== $this->options['subfolder']) {
            $sub_folder_path = apply_filters('useyourdrive_set_subfolder_path', Helpers::apply_placeholders($this->options['subfolder'], $this), $this->options, $this);
        ...
        if (defined('FORCE_REFRESH')) {
            ...
            $this->reset_complete_cache(false);
        } else {
        ...
        if (!empty($_REQUEST['folderPath'])) {
            ...
            if (false === $this->_folderPath || null === $this->_folderPath || !is_array($this->_folderPath)) {
                ...
                $current_folder = $this->get_client()->get_folder($this->_lastFolder);
        ...
        switch ($_REQUEST['action']) {
            ...
            case 'useyourdrive-preview':
                if (false === $this->get_user()->can_preview()) {
                    exit();
                }
            ...
            case 'useyourdrive-create-entry':
                ...
                if (is_wp_error($file)) {
                    ...
                    echo json_encode(['result' => '1', 'msg' => $new_name.' '.esc_html__('was added', 'wpcloudplugins'), 'lastFolder' => $new_folder_id]);
                }
            ...
            case 'useyourdrive-shorten-url':
                ...
                $shortened_url = $this->get_client()->shorten_url($cached_node->get_entry(), $url);
            ...
            default:
                ...
                exit('Use-your-Drive: '.esc_html__('No valid AJAX request', 'wpcloudplugins'));
        }
    ...
    public function create_from_shortcode($atts)
    {
        ...
        if (false === $cached_shortcode) {
            ...
            if (empty($dir)) {
                $mydrive = $this->get_client()->get_my_drive();
                $dir = $mydrive['folder']->get_id();
            }
