...
class Upload
{
    ...
    public function do_upload_direct()
    {
        ...
        if (empty($current_entry_id)) {
            $googledrive_file->setParents([$upload_folder_id]);
            $request = $this->get_app()->get_drive()->files->create($googledrive_file, ['fields' => $this->get_client()->apifilefields, 'supportsAllDrives' => true, 'enforceSingleParent' => true]);
        } else {
            $request = $this->get_app()->get_drive()->files->update($current_entry_id, $googledrive_file, ['fields' => $this->get_client()->apifilefields, 'supportsAllDrives' => true, 'enforceSingleParent' => true]);
        }
    ...
    public function upload_convert()
    {
        ...
        $this->get_processor()->get_cache()->pull_for_changes($current_folder_id, true);
        ...
        try {
            ...
            $api_entry = $this->get_app()->get_drive()->files->copy($cachedentry->get_id(), $entry, ['fields' => $this->get_client()->apifilefields, 'supportsAllDrives' => true, 'enforceSingleParent' => true]);

            if (false !== $api_entry && null !== $api_entry) {
                $new_id = $api_entry->getId();
                // Remove file from Cache
                $deleted_entry = $this->get_app()->get_drive()->files->delete($cachedentry->get_id(), ['supportsAllDrives' => true]);
    ...
    public function upload_post_process()
    {
        ...
        foreach ($_uploaded_entries as $cachedentry) {
            ...
            set_transient('useyourdrive_upload_'.$cachedentry->get_id(), true, HOUR_IN_SECONDS);
        }
    ...
    public function do_encryption($file)
    {
        ...
        try {
            require_once USEYOURDRIVE_ROOTPATH.'/AESCrypt/AESCryptFileLib.php';

            require_once USEYOURDRIVE_ROOTPATH.'/AESCrypt/aes256/MCryptAES256Implementation.php';
    ...
    public function create_folder_structure($path)
    {
        ...
        foreach ($folders as $key => $name) {
            ...
            try {
                ...
                $api_entry = $this->get_app()->get_drive()->files->create($newfolder, ['fields' => $this->get_client()->apifilefields, 'supportsAllDrives' => true,  'enforceSingleParent' => true]);
