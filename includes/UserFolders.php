...
class UserFolders
{
    ...
    public function create_user_folder($userfoldername, $shortcode, $mswaitaftercreation = 0)
    {
        ...
        if (false === $userfolder) {
            $this->get_client()->get_cache()->pull_for_changes($parent_folder->get_id(), true, -1);
            $userfolder = $this->get_client()->get_cache()->get_node_by_name($userfoldername, $parent_folder);
        }
    ...
    public function remove_user_folder($user_id)
    {
        ...
        foreach ($useyourdrivelists as $list) {
            ...
            foreach ($api_files as $api_file) {
                $batch->add($this->get_app()->get_drive()->files->update($api_file->getId(), $updateentry, ['supportsAllDrives' => true,  'enforceSingleParent' => true]));
            }
        }
    ...
    public function get_guest_user_name()
    {
        ...
        return apply_filters('useyourdrive_private_folder_name_guests', esc_html__('Guests', 'wpcloudplugins').' - '.$user_folder_name, $this->get_processor());
    }
