...
class LinkUsers
{
    ...
    {
        wp_enqueue_script('UseyourDrive.PrivateFolders', USEYOURDRIVE_ROOTPATH.'/includes/js/LinkUsers.min.js', ['UseyourDrive'], USEYOURDRIVE_VERSION, true);

        include sprintf('%s/templates/admin/private_folders.php', USEYOURDRIVE_ROOTDIR);
    }
            ...
            case 'private_folder':
                ...
                if (isset($linked_data['foldertext'])) {
                    if (!isset($linked_data['accountid'])) {
                        $linked_account = $UseyourDrive->get_accounts()->get_primary_account();
                    } else {
                        $linked_account = $UseyourDrive->get_accounts()->get_account_by_id($linked_data['accountid']);
                    }
