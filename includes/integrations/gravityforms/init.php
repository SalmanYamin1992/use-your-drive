    ...
    public function custom_field_settings($position, $form_id)
    {
        if (1430 == $position) {
            ...
            <li class="useyourdrive_setting field_setting">
              <label for="field_wpcp_useyourdrive">Shortcode <?php echo gform_tooltip('form_field_'.$this->_slug); ?></label>
              <textarea id="field_wpcp_useyourdrive" class="large fieldwidth-3 fieldheight-2" onchange="SetFieldProperty('UseyourdriveShortcode', this.value)"></textarea>
              </br>
              <button class='button gform-button primary wpcp-shortcodegenerator useyourdrive'><?php esc_html_e('Build your shortcode', 'wpcloudplugins'); ?></button>
            </li>
            <?php
        }
    }
...
class GF_WPCP_Field extends \GF_Field
{
    ...
    public $defaultValue = '[useyourdrive class="gf_upload_box" mode="upload" upload="1" uploadrole="all" upload_auto_start="0" userfolders="auto" viewuserfoldersrole="none"]';
    ...
    {
        if (!empty($this->UseyourdriveShortcode)) {
