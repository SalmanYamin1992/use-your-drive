...
class Cache
{
    ...
    public function add_to_cache(EntryAbstract $entry)
    {
        ...
        if ('shared-with-me' === $entry->get_parent_id() && $cached_node->has_parent() && 'shared-with-me' !== $cached_node->get_parent()->get_id()) {
            error_log('[WP Cloud Plugin message]: NODE FOUND WITHOUT PARENT WHILE CACHE HAS PARENT: '.$entry->get_id());
        ...
        if ($parent_in_tree) {
            ...
            $this->get_processor()->get_client()->get_folder($parent_id, false);
        }
        ...
        if ($entry->is_shortcut()) {
            ...
            if (false === $shortcut_node) {
                ...
                $result = $this->get_processor()->get_client()->get_entry($target_id, false);
                ...
                if (empty($result)) {
                    ...
                    error_log('[WP Cloud Plugin message]: '.sprintf('Shortcut '.$entry->get_name().'('.$entry->get_id().') points to a removed shortcut'));
    ...
    public function pull_for_changes($folder_id, $force_update = false, $buffer = 10)
    {
        ...
        if (empty($folder_id)) {
            $folder_id = $this->get_processor()->get_shortcode_option('root');
        }
        ...
        if (true === $force && ($this->get_last_check_for_update($drive_id) > $current_time - $buffer)) {
        ...
        $result = $this->get_processor()->get_client()->get_changes($drive_id, $this->get_last_check_token($drive_id));

        if (empty($result)) {
            return $this->update_cache(false);
        }
