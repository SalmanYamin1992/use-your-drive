...
class Download
{
    ...
    public function redirect_to_content()
    {
        ...
        if (isset($headers['location'])) {
            ...
            do_action('useyourdrive_log_event', 'useyourdrive_error', $this->get_cached_node(), ['error' => 403, 'message' => 'Download via redirect URL denied by Google likely due to hitting limits of the Fair Use Policy. Plugin will try stream the content instead via your server']);

            error_log('[WP Cloud Plugin message]: '.sprintf('Google Error on line %s: Download redirect for %s denied by Google likely due to hitting limits of the Fair Use Policy. Plugin will try stream the content instead via your server.', __LINE__, $this->get_cached_node()->get_id()));
    ...
    public function download_content()
    {
        ...
        if ($size <= 0) {
            ...
            while ($i <= $size) {
                ...
                //Output the chunk
                sleep(1); // halt for 1 second to prevent 100% CPU usage
