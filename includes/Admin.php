...
class Admin
{
    ...
    public function LoadAdmin($hook)
    {
        ...
        if ($hook == $this->networksettingspage || $hook == $this->settingspage) {
            ...
            wp_enqueue_script('wp-color-picker-alpha', USEYOURDRIVE_ROOTPATH.'/vendors/wp-color-picker-alpha/wp-color-picker-alpha.min.js', ['wp-color-picker'], '3.0.0', true);
            wp_enqueue_style('wp-color-picker');
            wp_enqueue_script('jquery-ui-accordion');
            wp_enqueue_media();
            add_thickbox();
            wp_enqueue_script('UseyourDrive.Admin');
        }
        ...
        if ($hook == $this->dashboardpage) {
            ...
            wp_enqueue_style('UseyourDrive.Datatables');
            wp_dequeue_style('UseyourDrive');
            wp_dequeue_style('UseyourDrive.CustomStyle');
    ...
    public function add_admin_menu()
    {
        ...
        if (Helpers::check_user_role($this->settings['permissions_edit_settings'])) {
            ...
            $menuadded = true;
            $this->settingspage = add_submenu_page($this->plugin_options_key, 'Use-your-Drive - '.esc_html__('Settings'), esc_html__('Settings'), 'read', $this->plugin_options_key, [$this, 'load_settings_page']);
        }
    ...
    public function add_admin_network_menu()
    {
        ...
        $this->networksettingspage = add_submenu_page($this->plugin_network_options_key, 'Use-your-Drive - '.esc_html__('Settings'), esc_html__('Settings'), 'read', $this->plugin_network_options_key, [$this, 'load_settings_network_page']);
    ...
    public function save_settings_network()
    {
        if (current_user_can('manage_network_options')) {
            update_site_option('useyourdrive_purchaseid', $_REQUEST['use_your_drive_settings']['purchase_code']);
    ...
    {
    	return 'activated';
        $purchasecode = isset($this->settings['purchase_code']) ? $this->settings['purchase_code'] : '';
        if (is_multisite()) {
    ...
    public function get_plugin_activated_box()
    {
        ...
        $box_input = '<input type="hidden" name="use_your_drive_settings[purchase_code]" id="purchase_code" value="'.esc_attr($purchase_code).'">';
        ...
        if (empty($purchase_code)) {
            ...
            if (false === is_plugin_active_for_network(USEYOURDRIVE_SLUG) || true === is_network_admin()) {
                $box_text .= "</p><p><input id='wpcp-activate-button' type='button' class='simple-button blue' value='".esc_html__('Activate', 'wpcloudplugins')."' /><span><a href='#' onclick='jQuery(\".useyourdrive_purchasecode_manual\").slideToggle()'>".esc_html__('Or insert your purchase code manually and press Activate', 'wpcloudplugins').'</a></span></p> ';
                $box_text .= '<div class="useyourdrive_purchasecode_manual" style="display:none" ><h3>Activate manually</h3><input name="use_your_drive_settings[purchase_code]" id="purchase_code" class="useyourdrive-option-input-large" placeholder="XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX" value="'.esc_attr($purchase_code).'"><a href="https://florisdeleeuwnl.zendesk.com/hc/en-us/articles/201834487" target="_blank">'.esc_html__('Where can I find my purchase code?', 'wpcloudplugins').'</a></div>';
                $box_input = '';
            }
        } else {
            $box_text .= "</p><p><input id='wpcp-check-updates-button' type='button' class='simple-button blue' value='".esc_html__('Check for updates', 'wpcloudplugins')."' />";

            if (false === is_plugin_active_for_network(USEYOURDRIVE_SLUG) || true === is_network_admin()) {
                $box_text .= "<input id='wpcp-deactivate-license-button' type='button' class='simple-button default' value='".esc_html__('Deactivate License', 'wpcloudplugins')."' />";
            }
        }
    ...
    public function get_plugin_authorization_box(Account $account)
    {
        ...
        $refreshbutton = "<div type='button' class='wpcp-refresh-account-button simple-button red small' data-account-id='{$account->get_id()}' data-url='{$app->get_auth_url()}' title='".esc_html__('Refresh', 'wpcloudplugins')."'><i class='fas fa-redo' aria-hidden='true'></i><div class='wpcp-spinner'></div></div>";
    ...
    public function get_admin_notice($force = false)
    {
        if (version_compare(PHP_VERSION, '7.0') < 0) {
            echo '<div id="message" class="error"><p><strong>Use-your-Drive - Error: </strong>'.sprintf(esc_html__('You need at least PHP %s if you want to use this plugin', 'wpcloudplugins'), '7.0').'. '.
            ...
            echo '<div id="message" class="error"><p><strong>Use-your-Drive - Error: </strong>'.sprintf(esc_html__('Cannot create the cache directory %s, or it is not writable', 'wpcloudplugins'), '<code>'.USEYOURDRIVE_CACHEDIR.'</code>').'. '.
        ...
        if (!file_exists(USEYOURDRIVE_CACHEDIR.'/.htaccess')) {
            echo '<div id="message" class="error"><p><strong>Use-your-Drive - Error: </strong>'.sprintf(esc_html__('Cannot find .htaccess file in cache directory %s', 'wpcloudplugins'), '<code>'.USEYOURDRIVE_CACHEDIR.'</code>').'. '.
            ...
            if (current_user_can('manage_options') || current_user_can('edit_theme_options')) {
                ...
                foreach ($this->get_main()->get_accounts()->list_accounts() as $account_id => $account) {
                    if (false === $account->get_authorization()->has_access_token() || (false !== wp_next_scheduled('useyourdrive_lost_authorisation_notification', ['account_id' => $account_id]))) {
                        echo '<div id="message" class="error"><p><span class="dashicons dashicons-warning"></span>&nbsp;<strong>Use-your-Drive: </strong>'.sprintf(esc_html__("The plugin isn't longer linked to the %s account", 'wpcloudplugins'), '<strong>'.$account->get_email().'</strong>').'.</p>'.
    ...
    public function get_admin_notice_not_activated()
    {
        ...
        if ('index.php' == $pagenow || 'plugins.php' == $pagenow) {
            if (current_user_can('manage_options') || current_user_can('edit_theme_options')) {
                $location = get_admin_url(null, 'admin.php?page=UseyourDrive_settings');
                echo '<div id="message" class="error"><p><strong>Use-your-Drive: </strong>'.esc_html__('The plugin is not yet activated', 'wpcloudplugins').'. '.esc_html__('Please activate the plugin to manage the plugin settings', 'wpcloudplugins').'. '.
