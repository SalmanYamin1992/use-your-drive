...
class CacheRequest
{
    ...
    public function __construct(Processor $_processor, $request = null, $cache_name = null)
    {
        if (empty($request)) {
            $request = $_REQUEST;
        }
        ...
        if (empty($cache_name)) {
            $cache_name = Helpers::filter_filename($this->get_processor()->get_current_account()->get_id().'_'.$_processor->get_listtoken(), false).'_'.$request_hash.'_'.$this->get_user_identifier();
        }

        $this->_cache_name = 'request_'.$cache_name;
