    ...
    public function convert_api_entry($api_entry, $virtual_folder = false)
    {
        ...
        if (empty($api_entry->getParents()) && false === $this->is_virtual_folder()) {
            if ($api_entry->getDriveId() === $api_entry->getId()) {
                ...
                // else?
                error_log('[WP Cloud Plugin message]: Found an item without parent (orphaned): '.$api_entry->getId());
            }
        }
    ...
    public function create_save_as()
    {
        switch ($this->get_mimetype()) {
            ...
            case 'application/vnd.google-apps.spreadsheet':
                $save_as = [
                    'MS Excel document' => ['mimetype' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'extension' => 'xlsx', 'icon' => 'fa-file-excel'],
